...
export default function WalletScreen() {
  ...
  }, [authUser, setWalletBag, storeBag.length]);
  const bag = (authUser ? storeBag : storeBag.length > 0 ? storeBag : INITIAL_BAG) as BagItem[];
  ...
  const currentBalance = useMemo(() => {
    ...
    // Server wallet_balance is already the net balance after purchases.
    if (authUser) return gross;
    // The anonymous demo balance is gross, so account for local bag purchases.
    const spent = bag
      .filter(b => b.member === selectedMember && (b.status === 'active' || b.status === 'used'))
      .reduce((sum, b) => sum + b.pts, 0);
    return Math.max(0, gross - spent);
  }, [authUser, selectedMember, familyMembers, bag]);
  ...
  const handleResell = useCallback((item: BagItem) => {
    if (authUser) {
      Alert.alert('Not available yet', 'Reselling is not enabled for household rewards yet.');
      return;
    }
    Alert.alert(
      `Resell "${item.name}"?`,
  ...
  const handleGift = useCallback((item: BagItem) => {
    if (authUser) {
      Alert.alert('Not available yet', 'Gifting is not enabled for household rewards yet.');
      return;
    }
  ...
  const commitUse = useCallback((item: BagItem, note: string | null) => {
    if (authUser) {
      void (async () => {
        try {
          await householdApi.redeemReward(item.id);
          Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
          const today = new Date().toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' });
          setBag(prev => prev.map(b => b.id === item.id ? { ...b, status: 'used', usedDate: today, note: note ?? b.note } : b));
        } catch (error) {
          Alert.alert('Reward not redeemed', error instanceof Error ? error.message : 'Please try again.');
        }
      })();
      return;
    }
    ...
    showUndo(item, next);
  }, [addWalletTransaction, authUser, showUndo]);
