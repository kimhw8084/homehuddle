...
export default function MarketScreen() {
  ...
  const { marketItems: storeItems, setMarketItems, marketFlashSales: storeFlashSales, setMarketFlashSales, addMarketFlashSale, marketItemHistory, setMarketItemHistory } = useHuddleStore();
  const remoteHouseholdId = useHuddleStore(state => state.householdId);
  const remoteMemberId = useHuddleStore(state => state.currentMemberId);
  const purchaseInFlight = useRef(false);
  const purchaseRequests = useRef(new Map<string, string>());
  const [purchasing, setPurchasing] = useState(false);

  // Anonymous development sessions retain the seeded demonstration catalogue.
  useEffect(() => {
    if (authUser) return;
  ...
  }, [authUser, marketItemHistory.length, setMarketItemHistory, setMarketItems, storeItems.length]);

  const refreshRemoteRewards = useCallback(async () => {
    if (remoteHouseholdId) requestHouseholdRefresh(remoteHouseholdId);
  }, [remoteHouseholdId]);

  const items = authUser ? storeItems : storeItems.length > 0 ? storeItems : INITIAL_ITEMS;
  ...
  const storeCurrentMember = authUser ? familyMembers.find(m => m.id === remoteMemberId) : familyMembers.find(m => m.name === currentUserName) ?? familyMembers[0];
  const canManageRewards = !authUser || storeCurrentMember?.role === 'Parent';
  // Bridge store member to market's expected shape (balance = pointsEarned)
  const member = { ...storeCurrentMember, name: storeCurrentMember?.name ?? '', avatar: storeCurrentMember?.avatar ?? '👤', balance: storeCurrentMember?.stats.pointsEarned ?? 0 };
  ...
  const activeSales = useMemo(() => authUser ? [] : flashSales.filter(s => s.expiresAt > now), [authUser, flashSales, now]);
  ...
  const handleBuy = useCallback((item: MarketItem, effectivePrice: number) => {
    ...
    if (authUser) {
      ...
      if (!remoteMemberId) {
        Alert.alert('Household not ready', 'Finish household setup before purchasing rewards.');
        return;
      }
      void (async () => {
        purchaseInFlight.current = true;
        setPurchasing(true);
        const key = `${authUser.id}:${item.id}`;
        const requestId = purchaseRequests.current.get(key) ?? randomUUID();
        purchaseRequests.current.set(key, requestId);
        try {
          await householdApi.purchaseReward(item.id, remoteMemberId, requestId);
          purchaseRequests.current.delete(key);
          await refreshRemoteRewards();
          setRedeemState({ item, price: item.pts, prevBalance });
        } catch (error) {
  ...
  }, [addWalletBagItem, authUser, currentUser, member.balance, refreshRemoteRewards, remoteMemberId, updateMemberStats]);
  ...
  const handleRedeemDone = useCallback(() => {
    ...
    setRedeemState(null);
  }, [authUser, redeemState]);

  const handleSaveNew = useCallback((data: Omit<MarketItem, 'id' | 'priceHistory' | 'createdBy' | 'purchaseCount'>) => {
    if (authUser) {
      if (!remoteHouseholdId) {
        Alert.alert('Household not ready', 'Finish household setup before adding rewards.');
        return;
      }
      void (async () => {
        try {
          await householdApi.createReward({ householdId: remoteHouseholdId, title: data.name, cost: data.pts, description: data.desc });
          await refreshRemoteRewards();
        } catch (error) {
          Alert.alert('Reward not created', error instanceof Error ? error.message : 'Please try again.');
        }
      })();
      return;
    }
  ...
  }, [authUser, currentUser, refreshRemoteRewards, remoteHouseholdId]);

  const handleSaveEdit = useCallback((data: Omit<MarketItem, 'id' | 'priceHistory' | 'createdBy' | 'purchaseCount'>) => {
    if (!editItem) return;
    if (authUser) {
      void householdApi.updateReward({ id: editItem.id, title: data.name, cost: data.pts, description: data.desc })
        .then(() => { void refreshRemoteRewards(); setEditItem(null); })
        .catch(error => Alert.alert('Reward not updated', error instanceof Error ? error.message : 'Please try again.'));
      return;
    }
    const today = new Date().toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' });
    ...
    setEditItem(null);
  }, [authUser, editItem, refreshRemoteRewards, setItems]);

  const handleDelete = useCallback((item: MarketItem) => {
    if (authUser) {
      Alert.alert(`Archive "${item.name}"?`, 'Existing purchases remain in each member’s wallet.', [
        { text: 'Cancel', style: 'cancel' },
        { text: 'Archive', onPress: () => { void householdApi.updateReward({ id: item.id, title: item.name, cost: item.pts, description: item.desc, active: false })
          .then(refreshRemoteRewards).catch(error => Alert.alert('Unable to archive', error instanceof Error ? error.message : 'Please try again.')); } },
      ]);
      return;
    }
  ...
  }, [authUser, refreshRemoteRewards, marketItemHistory, setMarketItemHistory, setItems]);

  const handleRestock = useCallback((item: MarketItem, qty: number) => {
    if (authUser) { Alert.alert('Unlimited rewards', 'Shared rewards do not currently have stock limits.'); return; }
    const today = new Date().toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' });
  ...
  return (
    <View style={{ flex: 1, backgroundColor: C.bg }}>
      <SafeAreaView style={{ flex: 1 }}>
        {purchasing && <Text accessibilityLiveRegion="polite" style={{ padding: 12, textAlign: 'center', color: C.accent }}>Confirming your purchase…</Text>}
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 24, paddingBottom: 120 }}>
          ...
          {authUser && items.length === 0 && !searchQuery && (
            <View style={{ alignItems: 'center', paddingVertical: 52, paddingHorizontal: 28 }}>
              <View style={{ width: 76, height: 76, borderRadius: 24, backgroundColor: C.accentBg, alignItems: 'center', justifyContent: 'center', marginBottom: 16 }}>
                <Text style={{ fontSize: 36 }}>🎁</Text>
              </View>
              <Text style={{ fontSize: 18, fontWeight: '900', color: C.text, textAlign: 'center' }}>The reward shelf is ready</Text>
              <Text style={{ fontSize: 13, lineHeight: 19, color: C.subtext, textAlign: 'center', marginTop: 7 }}>
                {canManageRewards ? 'Add the first reward your household can work toward.' : 'A parent can add rewards for your household here.'}
              </Text>
            </View>
          )}
          <View style={{ gap: 0 }}>
      ...
      <View style={{ position: 'absolute', right: 24, bottom: 100, flexDirection: 'row', gap: 16 }}>
        {/* Add Item */}
        {canManageRewards && <TouchableOpacity
          activeOpacity={0.9}
