...
The fuller authored **Home and Chores screens are active again**. Chores reads household state and persists several core create/edit/archive/assignment/completion actions, but recurrence, sections/order metadata, and advanced flows are not fully audited. Home uses household review commands, while automations, appliance controls, and some dashboard content remain preview/local state. Treat the in-app notice as the current boundary and verify important changes after reloading. The hidden development bypass uses sample data throughout; tap its **PREVIEW** control and **Exit preview and sign in** to test persistence. See [current implementation status](implementation-status.md).
...
Expo Go 57 startup was checked on the Mac's iOS 26.5 simulator runtime. The September 23 screenshots predate the Home/Chores route correction; refresh Expo Go to load the restored screens. Actual iPhone interaction, email delivery, photo permissions, and two-device synchronization still need a device walkthrough.
