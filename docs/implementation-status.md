...
The owner's authored Home, Chores, Market, and Wallet screens and four-tab navigation remain the UI baseline from original commit `dcf30b3`, restored on main by `20b6528`. This checkpoint improves those routes in place; it does not replace them with the earlier simplified feature screens.
...
**Home and Chores still use the authored sample/local-state implementation.** They now say this explicitly in the UI. Their integration remains implementation work, not an account-setup blocker. Do not treat their chore completion, review, automation, or appliance actions as persisted household changes. The next slice must connect these existing screens in place, preserving their calendar, sections, sorting, recurrence editor, proof, and assignment interactions.

Local evidence: TypeScript passed; 84 app tests across 22 suites passed; 122 database assertions passed in both isolated PostgreSQL/WASM and native PostgreSQL; two additional native concurrent-purchase checks passed; lint has 0 errors and 793 warnings (restoration baseline: 798); strict production-module lint passed; Expo dependency alignment and iOS/Android/web JavaScript exports passed. A 390×844 headless-browser walkthrough rendered all four original tabs without page errors. Those screenshots use explicitly labeled sample data: this is not iPhone, signed-in two-device, full accessibility, or human design acceptance. Hosted CI must be checked for the published commit.
