...
Correction: the September 23 restoration mistakenly treated March 8 commit `dcf30b3` as the latest authored UI. The owner's Home and Chores work continued through April 30 commit `c098688`; the fuller later screens were retained under `features/demo/screens` during the production refactor. The active Home and Chores routes now point back to those preserved screens. The Market and Wallet routes retain their persisted implementation. Do not use `dcf30b3` as the current design baseline.
