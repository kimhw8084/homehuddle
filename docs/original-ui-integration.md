...
HomeHuddle's latest complete pre-restoration project tree is `4ddb6ac` (September 23), immediately before the mistaken restore `20b6528`. Its full authored screens are preserved under `features/demo/screens/`; the Home/Chores source copies were last updated for SDK 57 in `3f77db2`. April 30 commit `c098688` is an earlier major feature snapshot in the original route-file history, not the latest screen snapshot. The September 23 recovery incorrectly used March 8 commit `dcf30b3` instead. The active Home and Chores routes now point to the preserved later screens. Keep the four-tab structure, visual language, rich cards, calendar, filters, bottom sheets, wallet chart, inventory gestures, and reward editor. Backend integration is not permission to replace the screens with a smaller app.
